...
export class AppComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    }, 30 * 1000); // --> en Milisegundos = 30 segundos verifica el estado de sesion
  ...
  verificarExpiracionToken(): void {
    if (!this.servicioSessionStorage.VerificarTokenExpirado()) {
      ...
      Swal.fire({
        ...
        clearInterval(swalInterval); // Detener el intervalo al interactuar con SweetAlert
        if (result.isConfirmed) {
          ...
          // El usuario ha seleccionado "Cerrar Sesión"
          this.cerrarSesion();
        }
      });
    }
